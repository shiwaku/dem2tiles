...
/**
 * 背景地図。地理院の最適化ベクトルタイルを軸に、淡色 / 標準 / 写真 / 白図から選ぶ。
 *
 * 段彩と陰影を重ねるので、背景は地名と水系が読める程度に抑えたい。既定は淡色。
 * 写真は「この起伏は何か」を確かめるとき用。
 *
 * 実装は shiwaku/ksj-suigai-rireki-converter の viewer に倣う。ダークテーマは
 * スタイル中の色を明度反転して生成するので、淡色・標準のどちらでも同じ手順で済む。
 */
...
function recolor(src: StyleSpecification, fn: (s: string) => string): StyleSpecification {
  const style = structuredClone(src)
 ...
 * 同一構成なので、切り替えても注入したレイヤーの積み順は変わらない。
 */
